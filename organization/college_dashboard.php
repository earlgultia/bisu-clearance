<?php
// college_dashboard.php - College Organizations Dashboard for BISU Online Clearance System
// Location: C:\xampp\htdocs\clearance\organization\college_dashboard.php
...
function ensureOrganizationProofColumns($db)
{
    ...
    try {
        ...
        error_log("Error ensuring college organization proof columns: " . $e->getMessage());
    }
}
...
// Verify that this is a college organization
if ($org_type !== 'college') {
    header("Location: ../index.php");
    exit();
}
...
try {
    $db->query("SELECT so.*, 
                (SELECT college_name FROM college WHERE college_id = so.college_id) as college_name
                FROM student_organizations so
                WHERE so.org_id = :org_id");
    $db->bind(':org_id', $org_id);
    $org_details = $db->single();

    $college_id = $org_details['college_id'] ?? null;
    $college_name = $org_details['college_name'] ?? 'All Colleges';
...
if (isset($_POST['add_lacking_comment'])) {
    ...
    if ($clearance_id > 0 && $comment !== '') {
        try {
            ...
            header("Location: college_dashboard.php?tab=pending");
            exit();
        } catch (Exception $e) {
            $db->rollback();
            error_log("Error adding college lacking comment: " . $e->getMessage());
...
if (isset($_POST['process_clearance'])) {
    ...
    if ($org_clearance_id && $status) {
        try {
            ...
            $db->query("UPDATE organization_clearance SET 
                        status = :status, 
                        remarks = CONCAT(IFNULL(remarks, ''), ' | College Org: ', :remarks),
            ...
            $db->rollback();
            error_log("Error processing college clearance: " . $e->getMessage());
            $error = "Error: " . $e->getMessage();
        }
    }
}
...
if (isset($_POST['bulk_approve'])) {
    ...
    if (!empty($org_clearance_ids)) {
        try {
            ...
            $db->query("UPDATE organization_clearance SET 
                        status = 'approved', 
                        remarks = CONCAT(IFNULL(remarks, ''), ' | College Org (Bulk): ', :remarks),
                        processed_by = :processed_by, 
                        processed_date = NOW(),
                        updated_at = NOW()
                        WHERE org_clearance_id IN ($placeholders)");
            ...
            if ($db->execute()) {
                $db->commit();
                $_SESSION['success_message'] = count($org_clearance_ids) . " college organization clearance(s) approved successfully!";
...
if (isset($_POST['undo_approval'])) {
    ...
    if ($org_clearance_id) {
        try {
            ...
            $undo_remarks = "UNDO by College Org: Previous status was '" . $current['status'] . "' on " . date('Y-m-d H:i:s') . ". Reason: " . $reason;
            ...
            if ($db->execute()) {
                ...
                header("Location: college_dashboard.php?tab=undo");
                exit();
            } else {
                $db->rollback();
...
try {
    ...
    // This recovers cases where org rows were not generated during student application.
    ...
    if (!$db->execute()) {
        error_log("Warning: organization clearance sync failed for org_id=" . $org_id);
    }
    ...
    // Get pending clearances with college-specific filtering
    $query = "SELECT 
                oc.*,
    ...
    // Add college filter if this org is for a specific college
    if ($college_id) {
        $query .= " AND (
                        u.college_id = :college_id
                        OR NOT EXISTS (
    ...
    $query .= " ORDER BY oc.processed_date DESC
                LIMIT 10";
    ...
    if ($college_id) {
        $query .= " AND cr.college_id = :college_id";
    }

    $query .= " GROUP BY cr.course_id, cr.course_name, cr.course_code
    ...
    $query = "SELECT u.users_id, u.fname, u.lname, u.ismis_id, u.course_id, u.year_level, u.address, u.profile_picture,
    ...
    $query .= " ORDER BY u.year_level, u.lname, u.fname";
    ...
    $query = "SELECT course_id, course_name, course_code FROM course WHERE 1=1";
    if ($college_id) {
    ...
    error_log("Error fetching college organization data: " . $e->getMessage());
...
<head>
    ...
    <title>College Organizations Dashboard - BISU Online Clearance</title>
    ...
    <style>
        * {
        ...
        :root {
            ...
            --primary-glow: rgba(44, 62, 80, 0.2);
            --bg-primary: #ffffff;
            --bg-secondary: #f8fafc;
            --bg-tertiary: #f1f5f9;
            --text-primary: #1e293b;
            ...
            --card-shadow-hover: 0 10px 30px rgba(44, 62, 80, 0.08);
        ...
        .dark-mode {
            --primary: #5d6d7e;
            --primary-dark: #4a5b6b;
            --primary-light: #6d7e8f;
            --primary-soft: rgba(93, 109, 126, 0.15);
            --primary-glow: rgba(93, 109, 126, 0.25);
            --bg-primary: #1a1b2f;
            --bg-secondary: #22243e;
            --bg-tertiary: #2a2c4a;
            --text-primary: #f0f1fa;
            ...
            --card-shadow-hover: 0 10px 30px rgba(93, 109, 126, 0.15);
            --header-bg: linear-gradient(135deg, #2c3e50 0%, #1e2b37 100%);
            --sidebar-bg: #22243e;
            --input-bg: #2a2c4a;
            --input-border: #3d3f60;
            --success: #4ade80;
        ...
        body {
            min-height: 100dvh;
            background:
                radial-gradient(circle at 100% 0%, rgba(52, 73, 94, 0.12), transparent 36%),
                radial-gradient(circle at 0% 100%, rgba(44, 62, 80, 0.09), transparent 42%),
                var(--bg-secondary);
        ...
        .header {
            ...
            box-shadow: 0 10px 30px rgba(12, 21, 30, 0.22);
        }
...
<body>
    ...
    <header class="header">
        <div class="header-content">
            <div class="logo">
                ...
                <h2>College Organizations Dashboard</h2>
            </div>
            <div class="user-menu">
                <div class="user-info">
                    ...
                    <div class="user-details">
                        ...
                        <div class="user-role">College Organization</div>
                    </div>
                </div>
        ...
        <aside class="sidebar" id="orgSidebar">
            <div class="profile-section">
                <div class="profile-avatar">
                    <i class="fas fa-university" style="font-size: 3rem; line-height: 100px;"></i>
                </div>
                ...
                <div class="profile-badge">College Organization</div>
                <?php if ($college_name): ?>
                    <div class="college-badge">
                        <i class="fas fa-building-columns"></i> <?php echo htmlspecialchars($college_name); ?>
                    </div>
                <?php endif; ?>
            </div>
        ...
        <main class="content-area">
            ...
            <div id="dashboard" class="tab-content <?php echo $active_tab == 'dashboard' ? 'active' : ''; ?>">
                ...
                <?php if (!empty($course_stats)): ?>
                        <div class="section-card">
                            <div class="section-header">
                                <h2><i class="fas fa-book-open"></i> Top Courses</h2>
                                <button class="btn btn-primary" onclick="switchTab('courses')">View All</button>
                            </div>
                            <div class="course-stats-grid">
                                <?php foreach (array_slice($course_stats, 0, 4) as $course): ?>
                                        <div class="course-stat-card">
                                            ...
                                            <?php if ($course['total_clearances'] > 0): ?>
                                                    ...
                                                    <div style="text-align: center; margin-top: 8px;">
                                                        <small><?php echo round(($course['approved_count'] / $course['total_clearances']) * 100, 1); ?>% Approval Rate</small>
                                                    </div>
                                            <?php endif; ?>
                                        </div>
                                <?php endforeach; ?>
            ...
            <div id="pending" class="tab-content <?php echo $active_tab == 'pending' ? 'active' : ''; ?>">
                <div class="section-card">
                    <div class="section-header">
                        <h2><i class="fas fa-clock"></i> Pending College Organization Clearances</h2>
                    ...
                    <?php if (empty($pending_clearances)): ?>
                            <div class="empty-state">
                                ...
                                <p>All college organization clearances have been processed.</p>
                            </div>
                    <?php else: ?>
                            <div class="table-responsive">
                                                ...
                                                <tr data-course="<?php echo $clearance['course_id']; ?>"
                                                    ...
                                                    <td>
                                                        <div class="tooltip">
                                                            ...
                                                            <span
                                                                class="tooltiptext"><?php echo htmlspecialchars($clearance['college_name'] ?? 'N/A'); ?></span>
                                                        </div>
                                                    </td>
                                                    <td><span class="year-badge <?php echo getYearLevelBadge($clearance['year_level'] ?? ''); ?>"><?php echo htmlspecialchars($clearance['year_level'] ?? 'N/A'); ?></span></td>
                                                    <td>
    ...
    <script>
        ...
        const dashboardBackGuardKey = '__collegeDashboardBackGuard';
        ...
        async function viewStudentDetails(userId) {
            ...
            try {
                const response = await fetch(`../get_student_records.php?user_id=${encodeURIComponent(userId)}`, { headers: { 'Accept': 'application/json' } });
                if (!response.ok) throw new Error(`Request failed with status ${response.status}`);
                const data = await response.json();
                if (!data.success || !data.student) throw new Error(data.message || 'Student details could not be loaded.');
                    ...
                    ? records.map(record => `
                        <div class="record-item">
                            ...
                            <div class="record-meta">
                                ...
                                <span class="year-badge ${student.year_level ? '' : 'secondary'}">${escapeHtml(record.updated_label || record.updated_at || 'No date')}</span>
                            </div>
                            ${record.lacking_comment ? `<div class="record-note"><strong>Lacking:</strong> ${escapeHtml(record.lacking_comment)}</div>` : ''}
                            ${(record.student_proof_file || record.proof_file) ? `<div class="record-meta">${record.student_proof_file ? '<span class="badge badge-primary">Student proof uploaded</span>' : ''}${record.proof_file ? '<span class="badge badge-success">Office proof uploaded</span>' : ''}</div>` : ''}
                        </div>
                    `).join('')
                        ...
                        <div class="student-detail-hero">
                            ...
                            <div class="student-detail-meta">
                                ...
                                <div class="student-detail-tags">
                                    ...
                                    <span class="year-badge ${student.year_level ? 'primary' : 'secondary'}">${escapeHtml(student.year_level || 'N/A')}</span>
                                </div>
                            </div>
                        </div>
                        <div class="summary-grid">
                            <div class="summary-card"><strong>${summary.total_records ?? 0}</strong><span>Total records</span></div>
                            <div class="summary-card"><strong>${summary.approved_count ?? 0}</strong><span>Approved</span></div>
                            <div class="summary-card"><strong>${summary.pending_count ?? 0}</strong><span>Pending</span></div>
                            <div class="summary-card"><strong>${summary.rejected_count ?? 0}</strong><span>Rejected</span></div>
                        </div>
                        <div class="section-card" style="margin-bottom: 0;">
                            <div class="section-header"><h2><i class="fas fa-id-card"></i> Student Information</h2></div>
                            <div class="detail-list">
                                ...
                                <div class="detail-item"><span>Address</span><span>${escapeHtml(student.address || 'Not available')}</span></div>
                            </div>
                        </div>
                        <div class="section-card" style="margin-bottom: 0;">
                            <div class="section-header"><h2><i class="fas fa-list-check"></i> Recent Clearance Records</h2></div>
                            <div class="record-list">${recordMarkup}</div>
                        </div>
                    </div>
                `;
            } catch (error) {
                modalBody.innerHTML = `<div class="empty-state"><i class="fas fa-exclamation-circle"></i><h3>Unable to load student details</h3><p>${escapeHtml(error.message || 'Please try again.')}</p></div>`;
            }
        }
        ...
        function filterPending() {
            ...
            rows.forEach(row => {
                ...
                row.style.display = matchesCourse && matchesYear && matchesSemester && matchesSchoolYear && matchesSearch ? '' : 'none';
            });
        }
        ...
        function filterHistory() {
            ...
            rows.forEach(row => {
                ...
                row.style.display = matchesCourse && matchesYear && matchesStatus && matchesSearch ? '' : 'none';
            });
        }
        ...
        function filterStudents() {
            ...
            cards.forEach(card => {
                ...
                card.style.display = matchesCourse && matchesYear && matchesSearch ? 'block' : 'none';
            });
        }
