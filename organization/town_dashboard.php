<?php
// town_dashboard.php - Town Organizations Dashboard for BISU Online Clearance System
// Location: C:\xampp\htdocs\clearance\organization\town_dashboard.php
...
// Verify that this is a town organization
if ($org_type !== 'town') {
    header("Location: ../index.php");
    exit();
}
...
if (isset($_POST['process_clearance'])) {
    ...
    if ($org_clearance_id && $status) {
        try {
            ...
            $db->query("SELECT oc.*, c.users_id, u.fname, u.lname, u.ismis_id, u.course_id, u.address
                       FROM organization_clearance oc
                       JOIN clearance c ON oc.clearance_id = c.clearance_id
                       JOIN users u ON c.users_id = u.users_id
            ...
            $db->query("UPDATE organization_clearance SET 
                        status = :status, 
                        remarks = CONCAT(IFNULL(remarks, ''), ' | Town Org: ', :remarks),
            ...
            $db->rollback();
            error_log("Error processing town clearance: " . $e->getMessage());
            $error = "Error: " . $e->getMessage();
        }
    }
}
...
if (isset($_POST['bulk_approve'])) {
    ...
    if (!empty($org_clearance_ids)) {
        try {
            ...
            $db->query("UPDATE organization_clearance SET 
                        status = 'approved', 
                        remarks = CONCAT(IFNULL(remarks, ''), ' | Town Org (Bulk): ', :remarks),
                        processed_by = :processed_by, 
                        processed_date = NOW(),
                        updated_at = NOW()
                        WHERE org_clearance_id IN ($placeholders)");
            ...
            if ($db->execute()) {
                $db->commit();
                $_SESSION['success_message'] = count($org_clearance_ids) . " town organization clearance(s) approved successfully!";
...
if (isset($_POST['undo_approval'])) {
    ...
    if ($org_clearance_id) {
        try {
            ...
            $undo_remarks = "UNDO by Town Org: Previous status was '" . $current['status'] . "' on " . date('Y-m-d H:i:s') . ". Reason: " . $reason;
            ...
            if ($db->execute()) {
                ...
                header("Location: town_dashboard.php?tab=undo");
                exit();
            } else {
                $db->rollback();
...
try {
    ...
    if (!empty($filter_barangay)) {
        $query .= " AND u.address LIKE :barangay";
    ...
    $query .= " ORDER BY oc.created_at ASC";
    ...
    $db->query("SELECT oc.*, 
                       c.users_id,
                       u.fname, u.lname, u.ismis_id, cr.course_name,
                       ct.clearance_name as clearance_type,
    ...
    $db->query("SELECT 
                ...
                AND u.address != ''
                GROUP BY barangay
                HAVING barangay IS NOT NULL
                ORDER BY barangay");
    $db->bind(':org_id', $org_id);
    $barangay_stats = $db->resultSet();

    // Get all students
    $db->query("SELECT users_id, fname, lname, ismis_id, course_id, address, profile_picture,
                       SUBSTRING_INDEX(address, ',', 1) as barangay
                FROM users 
    ...
    $db->query("SELECT DISTINCT SUBSTRING_INDEX(address, ',', 1) as barangay 
                FROM users 
                WHERE address IS NOT NULL AND address != ''
                AND user_role_id = (SELECT user_role_id FROM user_role WHERE user_role_name = 'student')
                ORDER BY barangay");
    $stats['barangays'] = $db->resultSet();

} catch (Exception $e) {
    error_log("Error fetching town organization data: " . $e->getMessage());
...
function formatBarangay($address)
{
    if (empty($address))
        return 'Unknown';
    ...
    <style>
        * {
        ...
        :root {
            --primary: #b45f2e;
            --primary-dark: #9e4f24;
            --primary-light: #c47b4f;
            --primary-soft: rgba(180, 95, 46, 0.1);
            --primary-glow: rgba(180, 95, 46, 0.2);
            --bg-primary: #ffffff;
            --bg-secondary: #f8fafc;
            --bg-tertiary: #f1f5f9;
            --text-primary: #1e293b;
            ...
            --card-shadow-hover: 0 10px 30px rgba(180, 95, 46, 0.08);
            --header-bg: linear-gradient(135deg, #b45f2e 0%, #c47b4f 100%);
            --sidebar-bg: #ffffff;
            --input-bg: #ffffff;
            --input-border: #e2e8f0;
            --success: #10b981;
        ...
        .dark-mode {
            ...
            --primary-soft: rgba(246, 173, 85, 0.15);
            --primary-glow: rgba(246, 173, 85, 0.25);
            --bg-primary: #1a1b2f;
            --bg-secondary: #22243e;
            --bg-tertiary: #2a2c4a;
            --text-primary: #f0f1fa;
            ...
            --card-shadow-hover: 0 10px 30px rgba(246, 173, 85, 0.15);
            --header-bg: linear-gradient(135deg, #b45f2e 0%, #9e4f24 100%);
            --sidebar-bg: #22243e;
            --input-bg: #2a2c4a;
            --input-border: #3d3f60;
            --success: #4ade80;
...
<body>
    ...
    <header class="header">
        <div class="header-content">
            <div class="logo">
                ...
                <h2>Town Organizations Dashboard</h2>
            </div>
            <div class="user-menu">
                <div class="user-info">
                    ...
                    <div class="user-details">
                        ...
                        <div class="user-role">Town Organization</div>
                    </div>
                </div>
    ...
    <div class="main-container">
        <aside class="sidebar" id="orgSidebar">
            <div class="profile-section">
                ...
                <div class="profile-badge">Town Organization</div>
                <div class="org-badge"><i class="fas fa-building"></i> Community Service</div>
            </div>
            ...
            <nav class="nav-menu">
                ...
                <button class="nav-item <?php echo $active_tab == 'barangays' ? 'active' : ''; ?>"
                    onclick="switchTab('barangays')">
                    <i class="fas fa-map-marked-alt"></i> Barangay Stats
                </button>
        ...
        <main class="content-area">
            ...
            <div id="dashboard" class="tab-content <?php echo $active_tab == 'dashboard' ? 'active' : ''; ?>">
                ...
                <!-- Top Barangays -->
                <?php if (!empty($barangay_stats)): ?>
                    <div class="section-card">
                        <div class="section-header">
                            <h2><i class="fas fa-map-marked-alt"></i> Top Barangays</h2>
                            <button class="btn btn-primary" onclick="switchTab('barangays')">View All</button>
                        </div>
                        <div class="barangay-stats-grid">
                            <?php foreach (array_slice($barangay_stats, 0, 4) as $barangay): ?>
                                <div class="barangay-card">
                                    <div class="barangay-name">
                                        <i class="fas fa-map-pin"></i>
                                        <?php echo htmlspecialchars($barangay['barangay'] ?: 'Unknown'); ?>
                                    </div>
                                    <div class="barangay-stats">
                                        <span class="label"><i class="fas fa-users"></i> Students:</span>
                                        <span class="value"><?php echo $barangay['student_count']; ?></span>
                                    </div>
                                    <div class="barangay-stats">
                                        <span class="label"><i class="fas fa-file-alt"></i> Clearances:</span>
                                        <span class="value"><?php echo $barangay['total_clearances']; ?></span>
                                    </div>
                                    <div class="barangay-stats">
                                        <span class="label"><i class="fas fa-check-circle" style="color: var(--success);"></i>
                                            Approved:</span>
                                        <span class="value"
                                            style="color: var(--success);"><?php echo $barangay['approved_count']; ?></span>
                                    </div>
                                    <?php if ($barangay['total_clearances'] > 0): ?>
                                        <div class="progress-bar">
                                            <div class="progress-fill"
                                                style="width: <?php echo ($barangay['approved_count'] / $barangay['total_clearances']) * 100; ?>%">
                                            </div>
                                        </div>
                                    <?php endif; ?>
                                </div>
            ...
            <div id="pending" class="tab-content <?php echo $active_tab == 'pending' ? 'active' : ''; ?>">
                <div class="section-card">
                    <div class="section-header">
                        <h2><i class="fas fa-clock"></i> Pending Town Organization Clearances</h2>
                    ...
                    <div class="filter-bar">
                        <select class="filter-select" id="barangayFilter">
                            ...
                            <?php foreach ($stats['barangays'] ?? [] as $barangay): ?>
                    ...
                    <?php if (empty($pending_clearances)): ?>
                        <div class="empty-state">
                            ...
                            <p>All town organization clearances have been processed.</p>
                        </div>
                    <?php else: ?>
                        <div class="table-responsive">
            ...
            <!-- Barangay Stats Tab -->
            <div id="barangays" class="tab-content <?php echo $active_tab == 'barangays' ? 'active' : ''; ?>">
                <div class="section-card">
                    <div class="section-header">
                        <h2><i class="fas fa-map-marked-alt"></i> Barangay Statistics</h2>
                    </div>

                    <?php if (empty($barangay_stats)): ?>
                        <div class="empty-state">
                            <i class="fas fa-map-marked-alt"></i>
                            <h3>No barangay data available</h3>
                            <p>Student addresses will appear here once they apply for clearance.</p>
                        </div>
                    <?php else: ?>
                        <div class="barangay-stats-grid">
                            <?php foreach ($barangay_stats as $barangay): ?>
                                <div class="barangay-card">
                                    <div class="barangay-name">
                                        <i class="fas fa-map-pin"></i>
                                        <?php echo htmlspecialchars($barangay['barangay'] ?: 'Unknown'); ?>
                                    ...
                                    <div class="barangay-stats">
                                        <span class="label"><i class="fas fa-file-alt"></i> Total:</span>
                                        <span class="value"><?php echo $barangay['total_clearances']; ?></span>
                                    </div>
                                    <div class="barangay-stats">
                                        <span class="label"><i class="fas fa-check-circle" style="color: var(--success);"></i>
                                    ...
                                    <div class="barangay-stats">
                                        <span class="label"><i class="fas fa-clock" style="color: var(--warning);"></i>
                                            Pending:</span>
                                        <span class="value"
                                            style="color: var(--warning);"><?php echo $barangay['pending_count']; ?></span>
                                    </div>
                                    <div class="barangay-stats">
                                        <span class="label"><i class="fas fa-times-circle" style="color: var(--danger);"></i>
                                            Rejected:</span>
                                        <span class="value"
                                            style="color: var(--danger);"><?php echo $barangay['rejected_count'] ?? 0; ?></span>
                                    </div>
                                    <?php if ($barangay['total_clearances'] > 0): ?>
                                        <div class="progress-bar">
                                            <div class="progress-fill"
                                        ...
                                        <div style="text-align: center; margin-top: 10px;">
                                            <small><?php echo round(($barangay['approved_count'] / $barangay['total_clearances']) * 100, 1); ?>%
                                                Approval Rate</small>
                                        </div>
                                    <?php endif; ?>
                                    <div style="margin-top: 15px;">
                                        <button class="btn btn-primary" style="width: 100%; padding: 8px;"
                                            onclick="filterByBarangay('<?php echo htmlspecialchars($barangay['barangay']); ?>')">
            ...
            <div id="history" class="tab-content <?php echo $active_tab == 'history' ? 'active' : ''; ?>">
                <div class="section-card">
                    ...
                    <!-- Filter Bar -->
                    <div class="filter-bar">
                        <select class="filter-select" id="historyBarangayFilter">
                            <option value="">All Barangays</option>
                            <?php foreach ($stats['barangays'] ?? [] as $barangay): ?>
                                <option value="<?php echo htmlspecialchars($barangay['barangay']); ?>">
                                    <?php echo htmlspecialchars($barangay['barangay']); ?></option>
                            <?php endforeach; ?>
                        </select>
            ...
            <div id="students" class="tab-content <?php echo $active_tab == 'students' ? 'active' : ''; ?>">
                <div class="section-card">
                    ...
                    <!-- Search Bar -->
                    <div class="filter-bar">
                        <select class="filter-select" id="studentBarangayFilter">
                            <option value="">All Barangays</option>
                            <?php foreach ($stats['barangays'] ?? [] as $barangay): ?>
                                <option value="<?php echo htmlspecialchars($barangay['barangay']); ?>">
                                    <?php echo htmlspecialchars($barangay['barangay']); ?></option>
                            ...
                            <div class="student-card"
                                ...
                                <div style="display: flex; align-items: center; gap: 15px; margin-bottom: 15px;">
                                    ...
                                    <div class="student-info">
                                        ...
                                        <span
                                            class="barangay-badge"><?php echo htmlspecialchars(formatBarangay($student['address'] ?? '')); ?></span>
                                    </div>
                                </div>
                                <div style="display: flex; gap: 10px;">
    ...
    <script>
        ...
        const dashboardBackGuardKey = '__townDashboardBackGuard';
        ...
        function filterByBarangay(barangay) {
            switchTab('pending');
            document.getElementById('barangayFilter').value = barangay;
            filterPending();
        }
        ...
        function filterPending() {
            ...
            rows.forEach(row => {
                ...
                row.style.display = matchesBarangay && matchesSemester && matchesSchoolYear && matchesSearch ? '' : 'none';
            });
        }
        ...
        function filterHistory() {
            const barangay = document.getElementById('historyBarangayFilter').value;
            ...
            rows.forEach(row => {
                ...
                row.style.display = matchesBarangay && matchesSemester && matchesStatus && matchesSearch ? '' : 'none';
            });
        }

        function clearHistoryFilters() {
            document.getElementById('historyBarangayFilter').value = '';
        ...
        function filterStudents() {
            ...
            cards.forEach(card => {
                const cardBarangay = card.getAttribute('data-barangay') || '';
                ...
                card.style.display = matchesBarangay && matchesSearch ? 'block' : 'none';
            });
        }

        function clearStudentFilters() {
            document.getElementById('studentBarangayFilter').value = '';
