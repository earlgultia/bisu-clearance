<?php
// registrar_dashboard.php - Enhanced Registrar Dashboard with Lacking/Proof/Undo Functionality
// Location: C:\xampp\htdocs\clearance\sub_admin\registrar_dashboard.php
...
// Check if user has Registrar office
$is_registrar = false;
$registrar_office_id = null;
$registrar_office_order = 5; // Registrar is step 5

if ($office_check && $office_check['office_name'] === 'Registrar') {
    $is_registrar = true;
    $registrar_office_id = $office_check['office_id'];
    $registrar_office_order = $office_check['office_order'] ?? 5;
}

if (!$is_registrar) {
    // Not authorized for registrar dashboard
    header("Location: ../index.php");
    exit();
}

// Get registrar information from session
$registrar_id = $_SESSION['user_id'];
$registrar_name = $_SESSION['user_name'] ?? '';
$registrar_email = $_SESSION['user_email'] ?? '';
$registrar_fname = $_SESSION['user_fname'] ?? '';
$registrar_lname = $_SESSION['user_lname'] ?? '';
...
function detectRegistrarProofUploadMimeType($tmpFilePath)
{
...
function saveRegistrarOptimizedProofImage($sourcePath, $destinationPath, $mimeType, $maxDimension = 1920, $jpegQuality = 82, $pngCompression = 7)
{
...
if (isset($_POST['undo_approval'])) {
    ...
    if ($clearance_id) {
        try {
            ...
            // Verify this clearance belongs to registrar office
            if ($current['office_id'] != $registrar_office_id) {
            ...
            $undo_remarks = "UNDO: Previous status was '" . $current['status'] . "' on " . date('Y-m-d H:i:s') . ". Reason: " . $reason . ". Undone by: " . $registrar_name;
            ...
            if ($db->execute()) {
                ...
                if (class_exists('ActivityLogModel')) {
                    ...
                    $logModel->log($registrar_id, 'UNDO_APPROVAL', "Undid " . $current['status'] . " for clearance ID: $clearance_id for student: {$current['fname']} {$current['lname']} ({$current['ismis_id']}). Reason: $reason");
                }

                $db->commit();
                $_SESSION['success_message'] = "Approval successfully undone! Clearance has been returned to pending status.";
                header("Location: registrar_dashboard.php?tab=undo");
                exit();
            } else {
                $db->rollback();
    ...
    if ($clearance_id && !empty($comment)) {
        try {
            ...
            if ($column_exists) {
                ...
                $db->bind(':commented_by', $registrar_id);
                $db->bind(':id', $clearance_id);
            } else {
            ...
            if ($db->execute()) {
                ...
                if (class_exists('ActivityLogModel')) {
                    $logModel = new ActivityLogModel();
                    $logModel->log($registrar_id, 'LACKING_COMMENT', "Added lacking comment for clearance ID: $clearance_id for student: {$current['fname']} {$current['lname']}. Comment: $comment");
                }
    ...
    if ($clearance_id) {
        try {
            ...
            if ($db->execute()) {
                ...
                if (class_exists('ActivityLogModel')) {
                    $logModel = new ActivityLogModel();
                    $logModel->log($registrar_id, 'CLEAR_LACKING', "Cleared lacking comment for clearance ID: $clearance_id for student: {$current['fname']} {$current['lname']}");
                }
...
// ============================================
// UPLOAD PROOF FUNCTIONALITY (for student's proof of lacking items)
// ============================================
if (isset($_POST['upload_student_proof'])) {
    $clearance_id = $_POST['clearance_id'] ?? '';
    ...
    if ($clearance_id && isset($_FILES['proof_file']) && $_FILES['proof_file']['error'] === UPLOAD_ERR_OK) {
        ...
        $detected_mime_type = detectRegistrarProofUploadMimeType($tmp_file_path);
        ...
        if (!$is_image_upload && !$is_pdf_upload) {
            ...
            try {
                ...
                $upload_dir = __DIR__ . '/../uploads/proofs/registrar/';
                if (!file_exists($upload_dir) && !mkdir($upload_dir, 0777, true)) {
                    throw new Exception("Unable to create registrar proof upload directory");
                }

                // Generate unique filename
                $extension = $is_pdf_upload ? 'pdf' : $allowed_image_types[$detected_mime_type];
                $filename = 'proof_registrar_' . $clearance_id . '_' . time() . '.' . $extension;
                $filepath = 'uploads/proofs/registrar/' . $filename;
                $destination_path = $upload_dir . $filename;

                $upload_saved = false;
                if ($is_image_upload) {
                    $upload_saved = saveRegistrarOptimizedProofImage(
                        $tmp_file_path,
                        $destination_path,
                        $detected_mime_type,
                        1920,
                ...
                if ($upload_saved) {
                    ...
                    // Check if the proof_file column exists
                    $db->query("SHOW COLUMNS FROM clearance LIKE 'proof_file'");
                    $column_exists = $db->single();
                    ...
                    if ($column_exists) {
                        ...
                        $db->query("UPDATE clearance SET 
                                    ...
                                    proof_uploaded_by = :uploaded_by,
                                    lacking_comment = NULL,
                                    updated_at = NOW()
                                    WHERE clearance_id = :id");
                        $db->bind(':proof_file', $filepath);
                        $db->bind(':remarks', $remarks);
                        $db->bind(':uploaded_by', $registrar_id);
                        $db->bind(':id', $clearance_id);
                    } else {
                        // Fallback to using remarks
                        $db->query("UPDATE clearance SET 
                                    remarks = CONCAT(IFNULL(remarks, ''), ' | PROOF UPLOADED: ', :remarks, ' - File: ', :proof_file),
                                    updated_at = NOW()
                                    WHERE clearance_id = :id");
                        $db->bind(':remarks', $remarks);
                    ...
                    if ($db->execute()) {
                        ...
                        if (class_exists('ActivityLogModel')) {
                            $logModel = new ActivityLogModel();
                            $logModel->log($registrar_id, 'UPLOAD_PROOF', "Uploaded proof for clearance ID: $clearance_id");
                        }

                        $db->commit();
                        $_SESSION['success_message'] = "Proof uploaded successfully! The lacking comment has been cleared.";
                        header("Location: registrar_dashboard.php?tab=pending");
                        exit();
                    } else {
                        $db->rollback();
...
if (isset($_POST['approve_clearance'])) {
    ...
    if ($clearance_id) {
        try {
            ...
            // Check if previous offices have approved (Librarian, SAS, Dean, Cashier)
            ...
            $db->bind(':current_order', $registrar_office_order);
            $prev_approved = $db->single();

            if ($prev_approved['approved_count'] < 4) { // Librarian, SAS, Dean, Cashier (4 offices)
                $db->rollback();
                $error = "Cannot approve yet. Previous offices (Librarian, SAS, Dean, and Cashier) must approve first.";
            ...
            $db->query("UPDATE clearance SET 
                        ...
                        updated_at = NOW()
                        WHERE clearance_id = :id");
            $db->bind(':processed_by', $registrar_id);
            $db->bind(':id', $clearance_id);

            if ($db->execute()) {
                // Check if all offices are now approved (clearance completed)
                ...
                if (class_exists('ActivityLogModel')) {
                    $logModel = new ActivityLogModel();
                    $logModel->log($registrar_id, 'APPROVE_CLEARANCE', "Approved clearance ID: $clearance_id for student: {$current['fname']} {$current['lname']} ({$current['ismis_id']})");
                }
...
if (isset($_POST['bulk_approve'])) {
    ...
    if (!empty($clearance_ids)) {
        try {
            ...
            $eligible_ids = array_column($eligible, 'clearance_id');

            if (count($eligible_ids) != count($clearance_ids)) {
                $invalid_count = count($clearance_ids) - count($eligible_ids);
                $warning = $invalid_count . " clearance(s) were skipped because previous offices haven't approved.";
            }

            if (empty($eligible_ids)) {
                $db->rollback();
                $error = "No valid clearances for bulk approval. Previous offices must approve first.";
            ...
            foreach ($eligible_ids as $index => $id) {
...
$student_summary = [
    'total' => 0,
    'with_registrar_records' => 0,
    'with_pending_registrar' => 0,
    'with_approved_registrar' => 0,
    'completed' => 0
];
...
try {
    ...
    // Get Registrar office ID (already have from above)
    if (!$registrar_office_id) {
        $db->query("SELECT office_id FROM offices WHERE office_name = 'Registrar'");
        $registrar_office = $db->single();
    ...
    // Get completed clearances (all offices approved)
    $db->query("SELECT COUNT(*) as count 
                FROM (
                    SELECT users_id, semester, school_year
                    FROM clearance
                    GROUP BY users_id, semester, school_year
                    HAVING SUM(CASE WHEN status = 'approved' THEN 1 ELSE 0 END) = COUNT(*)
                ) as completed");
    $result = $db->single();
    $stats['completed'] = $result ? (int) $result['count'] : 0;
    ...
    $params = [':office_id' => $registrar_office_id];
    ...
    $db->query("SELECT 
              ...
              LEFT JOIN clearance_type ct ON c.clearance_type_id = ct.clearance_type_id
              WHERE c.office_id = :office_id 
              AND c.processed_by = :registrar_id
              AND c.status IN ('approved', 'rejected')
              ORDER BY c.processed_date DESC");
    $db->bind(':office_id', $registrar_office_id);
    ...
    $query = "SELECT 
                ...
                cr.course_code,
                col.college_name,
                COALESCE(registrar.total_records, 0) as registrar_total_records,
                COALESCE(registrar.pending_count, 0) as registrar_pending_count,
                COALESCE(registrar.approved_count, 0) as registrar_approved_count,
                COALESCE(registrar.rejected_count, 0) as registrar_rejected_count,
                registrar.last_activity as registrar_last_activity,
                (SELECT COUNT(*) FROM clearance c WHERE c.users_id = u.users_id AND c.status = 'approved') as approved_count,
                (SELECT COUNT(*) FROM clearance c WHERE c.users_id = u.users_id AND c.status = 'approved' AND c.office_id = :completed_office_id) as completed_count
              FROM users u
              LEFT JOIN course cr ON u.course_id = cr.course_id
              LEFT JOIN college col ON u.college_id = col.college_id
              LEFT JOIN (
                    ...
                    FROM clearance
                    WHERE office_id = :registrar_office_id
                    GROUP BY users_id
              ) registrar ON registrar.users_id = u.users_id
              WHERE u.user_role_id = (SELECT user_role_id FROM user_role WHERE user_role_name = 'student')
              AND u.is_active = 1
              ORDER BY u.lname, u.fname";

    $db->query($query);
    $db->bind(':registrar_office_id', $registrar_office_id);
    $db->bind(':completed_office_id', $registrar_office_id);
    ...
    foreach ($students as $student) {
        ...
        if (($student['registrar_total_records'] ?? 0) > 0) {
            $student_summary['with_registrar_records']++;
        }

        if (($student['registrar_pending_count'] ?? 0) > 0) {
            $student_summary['with_pending_registrar']++;
        }

        if (($student['registrar_approved_count'] ?? 0) > 0) {
            $student_summary['with_approved_registrar']++;
        }

        // Check if clearance is fully completed (all offices approved)
        $db->query("SELECT COUNT(*) as total, 
                    SUM(CASE WHEN status = 'approved' THEN 1 ELSE 0 END) as approved 
                    FROM clearance 
                    WHERE users_id = :user_id");
        $db->bind(':user_id', $student['users_id']);
        $check = $db->single();

        if ($check && $check['total'] == $check['approved']) {
...
$dashboard_name_parts = preg_split('/\s+/', trim((string) $registrar_name));
$dashboard_first_name = $dashboard_name_parts[0] ?? 'Registrar';
...
// Helper function to check if previous offices are approved
function checkPreviousOffices($approved_count)
{
    return $approved_count >= 4; // Librarian, SAS, Dean, and Cashier (4 offices)
}
...
function getActivityIcon($action)
{
    $icons = [
        ...
        'CLEARANCE_COMPLETED' => 'check-double'
    ];
...
<head>
    ...
    <title>Registrar Dashboard - BISU Online Clearance</title>
    ...
    <style>
        * {
        ...
        :root {
            ...
            --completed-soft: rgba(139, 92, 246, 0.1);
        }

        .dark-mode {
            --primary: #9d85e0;
            --primary-dark: #8b6fd8;
            --primary-light: #b5a1e8;
            --primary-soft: rgba(157, 133, 224, 0.15);
            --primary-glow: rgba(157, 133, 224, 0.25);
            --bg-primary: #1a1b2f;
            --bg-secondary: #22243e;
            --bg-tertiary: #2a2c4a;
            --text-primary: #f0f1fa;
            ...
            --card-shadow-hover: 0 10px 30px rgba(157, 133, 224, 0.15);
            --header-bg: linear-gradient(135deg, #2a2c4a 0%, #3d3f60 100%);
            --sidebar-bg: #22243e;
            --input-bg: #2a2c4a;
            --input-border: #3d3f60;
            --success: #4ade80;
            ...
            --completed-soft: rgba(139, 92, 246, 0.15);
        }
        ...
        .dashboard-panel-icon.completed {
        ...
        .pending-table tbody tr:hover td {
            border-color: rgba(65, 40, 134, 0.25);
            box-shadow: 0 10px 30px rgba(65, 40, 134, 0.06);
        }
        ...
        .pending-row-waiting td:first-child {
...
<body>
            ...
            <div class="user-menu">
                <div class="user-info">
                    ...
                    <div class="user-details">
                        <div class="user-name">
                            <?php echo htmlspecialchars($registrar_name); ?>
                        </div>
        ...
        <aside class="sidebar" id="subAdminSidebar">
            <div class="profile-section">
                ...
                <div class="profile-email">
                    <?php echo htmlspecialchars($registrar_email); ?>
                </div>
                <div class="profile-badge">Registrar</div>
                <div class="office-badge"><i class="fas fa-archive"></i> Registrar's Office - Final Step</div>
            </div>
        ...
        <main class="content-area">
            ...
            <div id="dashboard" class="tab-content <?php echo $active_tab == 'dashboard' ? 'active' : ''; ?>">
                <div class="dashboard-hero">
                    <div class="welcome-banner">
                        <div class="dashboard-kicker">
                            <i class="fas fa-archive"></i> Registrar's Office - Final Step
                        </div>
                        <h1>Welcome back, Registrar <?php echo htmlspecialchars($dashboard_first_name); ?>.</h1>
                        <p>
                            <?php if (($pending_summary['total'] ?? 0) > 0): ?>
                                    You have <?php echo $pending_summary['total']; ?> clearance request(s) in the final queue,
                                    and <?php echo $pending_summary['ready_for_bulk']; ?> can be completed right now.
                            <?php else: ?>
                                    Your final queue is clear right now. Use the records and history tabs to review completed
                                    clearances and student progress.
                            <?php endif; ?>
                        </p>
                    ...
                    <div class="dashboard-hero-panel">
                        ...
                        <div class="dashboard-panel-subtitle">Start with the clearest next actions in the final queue.</div>
                        ...
                        <div class="dashboard-panel-list">
                            <div class="dashboard-panel-item">
                                ...
                                <div class="dashboard-panel-content">
                                    <strong><?php echo $pending_summary['ready_for_bulk']; ?> ready to complete</strong>
                                    <span>These rows have all previous offices approved and can be finalized after review.</span>
                                </div>
                            </div>
                            ...
                            <div class="dashboard-panel-item">
                                <div class="dashboard-panel-icon completed">
                                    <i class="fas fa-check-double"></i>
                                </div>
                                <div class="dashboard-panel-content">
                                    <strong><?php echo $stats['completed'] ?? 0; ?> completed</strong>
                                    <span>Total clearances fully processed through all offices.</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="stats-grid">
                    <div class="stat-card">
                        ...
                        <div class="stat-details">
                            ...
                            <p>Pending Queue</p>
                            <small>All requests currently waiting for a registrar action.</small>
                        </div>
                    </div>
                    <div class="stat-card">
                        ...
                        <div class="stat-details">
                            ...
                            <small>Rows eligible for final approval after you review them.</small>
                        </div>
                    </div>
                    <div class="stat-card">
                    ...
                    <div class="stat-card">
                        ...
                        <div class="stat-details">
                            ...
                            <p>Approved By Registrar</p>
                            <small>Total registrar approvals already processed.</small>
                        </div>
                    </div>
                    <div class="stat-card">
                        ...
                        <div class="stat-details">
                            ...
                            <p>Fully Completed</p>
                            <small>Clearances fully approved by all offices.</small>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="section-card">
                        ...
                        <?php else: ?>
                                <div class="dashboard-empty-state">
                                    ...
                                    <p>Your final queue is clear. New requests will appear here as they arrive.</p>
                                </div>
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="section-card">
                        ...
                        <div class="dashboard-action-grid">
                            <div class="dashboard-action-card">
                                ...
                                <div class="dashboard-action-meta">Student uploads that need a registrar check before you decide.</div>
                                <button class="btn btn-secondary" onclick="switchTab('pending')">
                            ...
                            <div class="dashboard-action-card">
                                <div class="dashboard-action-value"><?php echo $student_summary['with_pending_registrar']; ?></div>
                                <div class="dashboard-action-title">Students Waiting</div>
                                <div class="dashboard-action-meta">Students with at least one pending registrar record.</div>
                                <button class="btn btn-secondary" onclick="switchTab('students')">
                                    <i class="fas fa-users"></i> Student Records
                                </button>
                            </div>
                ...
                <div class="dashboard-split-grid">
                    <div class="section-card">
                        <div class="section-header">
                            <h2><i class="fas fa-check-circle"></i> Recent Registrar Decisions</h2>
                            <button class="btn btn-secondary" onclick="switchTab('history')">
                                            ...
                                            <div class="dashboard-list-item">
                                                ...
                                                <div class="dashboard-list-meta">
                                                    ...
                                                    <span><i class="fas fa-user"></i>
                                                        <?php echo htmlspecialchars(trim(($clearance['processed_fname'] ?? '') . ' ' . ($clearance['processed_lname'] ?? '')) ?: 'Registrar office'); ?></span>
                                                </div>
                                            </div>
                                    <?php endforeach; ?>
                                </div>
                        <?php else: ?>
                                <div class="dashboard-empty-state">
                                    ...
                                    <p>Approved and rejected registrar actions will appear here once processing starts.</p>
                                </div>
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="section-card">
                        ...
                        <?php else: ?>
                                <div class="dashboard-empty-state">
                                    ...
                                    <p>System and user actions will appear here once the registrar's office is active.</p>
                                </div>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div style="display: none;" aria-hidden="true">
                    <div class="welcome-banner">
                        <h1>Welcome, Registrar
                            <?php echo htmlspecialchars(explode(' ', $registrar_name)[1] ?? ''); ?>.
                        </h1>
                        <p>Manage registrar-level clearances and track student records.</p>
                ...
                <div class="section-card">
                    ...
                    <div class="summary-grid">
                        <div class="summary-card">
                            ...
                            <div class="meta">Current registrar requests waiting in the queue</div>
                        </div>
                        <div class="summary-card">
                    ...
                    <div class="filter-bar">
                        ...
                        <select class="filter-select" id="pendingStateFilter">
                            ...
                            <option value="awaiting-previous">Waiting for Previous Offices</option>
                        </select>
                    ...
                    <?php else: ?>
                                    ...
                                    <tbody>
                                        <?php foreach ($pending_clearances as $clearance): ?>
                                                ...
                                                if (!$prev_approved) {
                                                    ...
                                                    $pending_state_class = 'waiting';
                                                    $pending_row_class = 'pending-row-waiting';
                                                    $pending_state_meta = 'Approval is locked until previous offices (Librarian, SAS, Dean, Cashier) approve.';
                                                    ...
                                                    <td>
                                                        <div class="pending-detail-stack">
                                                            ...
                                                            <?php elseif (!$prev_approved): ?>
                                                                    <div class="pending-meta">Waiting for Librarian, SAS, Dean, and Cashier approval first.</div>
                                                            <?php endif; ?>
                                                        </div>
                                                    </td>
                                                    <td>
                            ...
                            <div style="margin-top: 20px; padding: 15px; background: var(--success-soft); border-radius: 12px;">
                                <p style="color: var(--success);">
                                    ...
                                    still locked until proof arrives. <span class="badge badge-warning">Waiting for previous offices</span> indicates previous offices haven't approved yet.
                                </p>
                            </div>
                    <?php endif; ?>
                </div>
                ...
                <div class="section-card">
                    ...
                    <div class="summary-grid">
                        <div class="summary-card">
                            ...
                            <div class="meta">Latest 50 registrar clearance entries</div>
                        </div>
                        <div class="summary-card">
                            <div class="label">Approved</div>
                            <div class="value"><?php echo $history_summary['approved']; ?></div>
                            <div class="meta">Processed successfully by the registrar's office</div>
                        </div>
                        <div class="summary-card">
                            <div class="label">Pending</div>
                            <div class="value"><?php echo $history_summary['pending']; ?></div>
                            <div class="meta">Still awaiting a final registrar action</div>
                        </div>
                        <div class="summary-card">
                            <div class="label">Rejected</div>
                        ...
                        <div class="summary-card">
                            ...
                            <div class="meta">Records with student or registrar proof attached</div>
                        </div>
                    </div>
                    ...
                    <div class="section-tools">
                        ...
                        <div class="filter-meta">Use period, type, and status filters to audit specific registrar actions.</div>
                    </div>
                ...
                <div class="section-card">
                    ...
                    <div class="summary-grid">
                        <div class="summary-card">
                            ...
                            <div class="meta">Active student accounts available to the registrar</div>
                        </div>
                        <div class="summary-card">
                            <div class="label">With Registrar Records</div>
                            <div class="value"><?php echo $student_summary['with_registrar_records']; ?></div>
                            <div class="meta">Students who already have registrar activity</div>
                        </div>
                        <div class="summary-card">
                            <div class="label">Pending At Registrar</div>
                            <div class="value"><?php echo $student_summary['with_pending_registrar']; ?></div>
                            <div class="meta">Students currently waiting for registrar action</div>
                        </div>
                        <div class="summary-card">
                            <div class="label">Approved At Registrar</div>
                            <div class="value"><?php echo $student_summary['with_approved_registrar']; ?></div>
                            <div class="meta">Students with at least one approved registrar record</div>
                        </div>
                        <div class="summary-card">
                            <div class="label">Fully Completed</div>
                            <div class="value"><?php echo $student_summary['completed']; ?></div>
                            <div class="meta">Students with all offices approved</div>
                        </div>
                    </div>
                                ...
                                <?php foreach ($students as $student): ?>
                                        <div class="student-card"
                                            ...
                                            <div class="student-badges">
                                                ...
                                                <?php endif; ?>
                                                <?php if (($student['registrar_pending_count'] ?? 0) > 0): ?>
                                                        <span class="badge badge-warning">
                                                            <i class="fas fa-clock"></i>
                                                            <?php echo (int) $student['registrar_pending_count']; ?> registrar pending
                                                        </span>
                                                <?php endif; ?>
                                                <?php if (($student['completed_count'] ?? 0) > 0 && ($student['completed_count'] ?? 0) == ($student['approved_count'] ?? 0)): ?>
                                            ...
                                            <div class="student-metrics">
                                                <div class="student-metric">
                                                    <span class="metric-label">Registrar Records</span>
                                                    <span class="metric-value"><?php echo (int) ($student['registrar_total_records'] ?? 0); ?></span>
                                                </div>
                                                <div class="student-metric">
                                                    <span class="metric-label">Registrar Pending</span>
                                                    <span class="metric-value"><?php echo (int) ($student['registrar_pending_count'] ?? 0); ?></span>
                                                </div>
                                                <div class="student-metric">
                                                    <span class="metric-label">Registrar Approved</span>
                                                    <span class="metric-value"><?php echo (int) ($student['registrar_approved_count'] ?? 0); ?></span>
                                                </div>
                                            </div>
                                            ...
                                            <div class="student-card-footer">
                                                <div class="student-last-activity">
                                                    <i class="fas fa-clock"></i>
                                                    Last registrar activity:
                                                    <?php echo !empty($student['registrar_last_activity']) ? date('M d, Y h:i A', strtotime($student['registrar_last_activity'])) : 'No registrar activity yet'; ?>
                                                </div>
                    ...
                    <div class="filter-empty-state" id="studentsNoResults">
                        ...
                        <p>Try a different name or ISMIS ID to find the student record you need.</p>
                    </div>
                </div>
            </div>
        </main>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="lacking_comment" id="lackingComment" rows="4"
                            placeholder="e.g., Incomplete requirements, Missing documents, Unpaid fees..." required></textarea>
                    </div>
    ...
    <!-- Upload Proof Modal (for student's proof of lacking items) -->
    <div id="proofModal" class="modal">
        <div class="modal-content">
            <div class="modal-header">
                <h3><i class="fas fa-paperclip"></i> Upload Student's Proof</h3>
            ...
            <form method="POST" action="" enctype="multipart/form-data" id="proofForm">
                <div class="modal-body">
                    ...
                    <div class="info-card" style="background: var(--proof-soft); border-color: var(--proof);">
                        <i class="fas fa-info-circle"></i>
                        <div>
                            <strong>Upload proof that the student has resolved the lacking items.</strong> This could be scanned documents, certificates, or receipts.
                        </div>
                    </div>

                    <div class="form-group">
                        <label><i class="fas fa-file"></i> Select File <span class="required">*</span></label>
                        <input type="file" name="proof_file" id="proofFile" class="form-control" accept=".pdf,.jpg,.jpeg,.png" required>
                        <small style="color: var(--text-muted);">Allowed: PDF, JPG, PNG (Max 5MB)</small>
                    </div>

                    <div class="form-group">
                        <label><i class="fas fa-comment"></i> Remarks (Optional)</label>
                        <textarea name="proof_remarks" id="proofRemarks" rows="3"
                            placeholder="e.g., Student has submitted all requirements."></textarea>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" onclick="closeProofModal()">Cancel</button>
                    <button type="submit" name="upload_student_proof" class="btn btn-proof">Upload Proof</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <!-- View Registrar Proof Modal -->
    <div id="viewRegistrarProofModal" class="modal">
        <div class="modal-content large">
            <div class="modal-header">
                <h3><i class="fas fa-file"></i> Registrar's Office Proof</h3>
                <button class="close" onclick="closeViewRegistrarProofModal()">&times;</button>
            </div>
            <div class="modal-body" id="viewRegistrarProofBody">
                <div class="student-info-card" id="registrarProofInfo"></div>
                <div class="form-group">
                    <label><i class="fas fa-file"></i> Proof File:</label>
                    <div id="registrarProofPreview" style="text-align: center;"></div>
                </div>
                <div class="form-group">
                    <label><i class="fas fa-comment"></i> Remarks:</label>
                    <div id="registrarProofRemarks"
                        style="background: var(--bg-secondary); padding: 15px; border-radius: 8px; border: 1px solid var(--border-color); white-space: pre-wrap;">
                    </div>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" onclick="closeViewRegistrarProofModal()">Close</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        const dashboardBackGuardKey = '__registrarDashboardBackGuard';
        ...
        // View Registrar Proof
        function viewRegistrarProof(proofFile, remarks, uploadedBy, date) {
            const safeProofFile = String(proofFile || '')
                .replace(/\\/g, '/')
                .replace(/^(\.\.\/|\.\/)+/, '')
                .replace(/^\/+/, '');
            ...
            document.getElementById('registrarProofPreview').innerHTML = previewHtml;
            document.getElementById('registrarProofRemarks').textContent = remarks || 'No remarks provided';
            document.getElementById('registrarProofInfo').innerHTML = `
                <p><strong>Uploaded by:</strong> ${uploadedBy || 'Unknown'}</p>
                <p><strong>Date:</strong> ${date ? new Date(date).toLocaleString() : 'Unknown'}</p>
            `;

            document.getElementById('viewRegistrarProofModal').style.display = 'flex';
        }

        function closeViewRegistrarProofModal() {
            document.getElementById('viewRegistrarProofModal').style.display = 'none';
        }
        ...
        window.onclick = function (event) {
            const modals = ['approveModal', 'lackingModal', 'proofModal', 'viewLackingModal', 'viewStudentProofModal', 'viewRegistrarProofModal', 'undoModal', 'progressModal'];
